...
// eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwiaWF0IjoxNzEyNTEyOTE5fQ.KxXGyjuzm1f0SaGtP1XIHLxYt8ibGLx6TGRiluW9_us
...
app.get("/api/checkout/:token", async (req, res, next) => {
    try {
        const user = await findUserWithToken(req.params.token);
        console.log("checkout:", user);
        const rows = await checkout(user.id);
...
app.post('/api/users/cart_products', async (req, res, next) => {
    try {
        ...
        console.log("cart_products:", userId);
        if (!userId) {
            res.send({ status: 400, message: "User Not Logged" });
            return;
        }
